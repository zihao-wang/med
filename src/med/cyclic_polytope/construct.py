...
def verify_construction_up_to_k(
    m: int,
    k: int,
    n: int,
    t: Optional[np.ndarray] = None,
    ...
    """Check the construction for every nonempty subset of size at most ``k``."""
    total_checked = 0
    total_failed = 0
    checks_by_size: dict[int, int] = {}
    for subset_size in range(1, k + 1):
        ...
        checks_by_size[subset_size] = checked
        total_checked += checked
        total_failed += failed
        if not ok:
            return False, total_checked, total_failed, checks_by_size
    return True, total_checked, total_failed, checks_by_size
